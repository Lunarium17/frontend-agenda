...
const listarContactos = async () => {
  cargando.value = true;
  try {
    const response = await axios.get('http://localhost/backend-api/api/contactos/index.php', {
...
const procesarFormulario = async () => {
  ...
  try {
    if(idEditar.value) {
      await axios.put(`http://localhost/backend-api/api/contactos/actualizar.php?id=${idEditar.value}`, form.value, {
        headers: { Authorization: `Bearer ${authStore.token}` }
      });
      alert("Contacto actualizado");
    } else {
      await axios.post('http://localhost/backend-api/api/contactos/crear.php', form.value, {
        headers: { Authorization: `Bearer ${authStore.token}` }
      });
...
const eliminar = async (id) => {
  ...
  try {
    await axios.delete(`http://localhost/backend-api/api/contactos/eliminar.php?id=${id}`, {
      headers: { Authorization: `Bearer ${authStore.token}` }
    });
    await listarContactos();
  } catch (error) {
